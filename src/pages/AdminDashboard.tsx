...
export default function AdminDashboard({
  ...
  const seedDatabase = async () => {
    if (
      !window.confirm(
        "Isto criará dados de exemplo (turmas, alunos e quizzes). Tem a certeza que deseja continuar?",
      )
    )
      return;
    ...
    const classRef = await addDoc(
      ...
      { name: "Turma Manhã - Info Básica", order: 0 },
    );
        ...
        questions: [
          {
            ...
            explanation: "Hardware é a parte física.",
            options: [
              { text: "Programas", isCorrect: false },
              { text: "Parte física", isCorrect: true },
              { text: "Internet", isCorrect: false },
              { text: "Arquivos", isCorrect: false },
            ],
          },
          {
            question: "O que é um Software?",
            explanation: "A parte lógica.",
            options: [
              { text: "Parte Lógica", isCorrect: true },
              { text: "Peças internas", isCorrect: false },
              { text: "Mesa", isCorrect: false },
              { text: "Cabo", isCorrect: false },
            ],
          },
        ],
      },
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-slate-200 min-h-[85vh] flex flex-col lg:flex-row overflow-hidden fade-in">
      ...
      <div className="w-full lg:w-72 bg-slate-900 text-slate-300 flex-shrink-0 flex flex-col border-r border-slate-800">
        <div className="p-6 border-b border-slate-800 flex justify-between items-center lg:block">
          ...
          <button
            ...
            className="lg:hidden text-xs bg-slate-800 text-slate-300 hover:text-white border border-slate-700 rounded-lg px-3 py-2 transition font-medium"
          >
            Auto-Dados
          </button>
        </div>
        ...
        <div className="p-6 mt-auto hidden lg:block border-t border-slate-800">
          <button
            onClick={seedDatabase}
            className="w-full text-sm font-medium bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 border border-slate-700 rounded-xl p-3 transition-colors flex items-center justify-center gap-2"
          >
      ...
      <div className="flex-1 p-5 md:p-10 overflow-y-auto bg-slate-50/50">
        ...
        {activeTab === "alunos" && (
          <AdminAlunos students={students} classes={classes} />
        )}
        ...
        {activeTab === "ranking" && (
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2">
              <Trophy className="text-yellow-500" size={24} /> Classificação
              Geral
            </h3>
            <RankingList
              reports={reports}
...
function AdminTurmas({ classes }: { classes: Turma[] }) {
  ...
  const handleDelete = async (id: string) => {
    if (
      window.confirm(
        "Tem a certeza? Pode quebrar os dados dos alunos associados.",
      )
    ) {
      await deleteDoc(
        doc(db, "artifacts", appId, "public", "data", "classes", id),
      );
    }
  };
  ...
  const handleSort = () => {
    ...
    _classes.forEach((c, index) => {
      if (c.order !== index) {
        updateDoc(
          doc(db, "artifacts", appId, "public", "data", "classes", c.id),
          { order: index },
        );
      }
    });
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">
          Crie e gira as turmas ou grupos de estudo. Arraste para reordenar.
        </p>
      </div>

      <form
        onSubmit={handleAdd}
        className="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-200 mb-8 flex flex-col sm:flex-row gap-3"
      >
        <input
          type="text"
          value={name}
          ...
          className="flex-1 p-3 px-4 rounded-xl border border-slate-300 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition-all"
          required
        />
        <button
          type="submit"
          className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-transform active:scale-95 shadow-sm"
        >
...
function AdminAlunos({
  ...
  const handleDelete = async (id: string) => {
    if (
      window.confirm(
        "Tem a certeza absoluta? O aluno perderá acesso a todo o histórico.",
      )
    ) {
      await deleteDoc(
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">
          Gira o acesso, palavras-passe e registo de estudantes na plataforma.
        </p>
      </div>
      ...
      >
        <div className="flex justify-between items-center mb-4 border-b border-slate-200/50 pb-3">
          <h4
            className={`font-bold flex items-center gap-2 ${editingId ? "text-orange-800" : "text-slate-800"}`}
          >
            {editingId ? (
              <Pencil size={18} />
            ) : (
          ...
          {editingId && (
            <button
              ...
              className="text-sm font-bold text-slate-500 hover:text-red-500 flex items-center gap-1"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
          <div className="md:col-span-4">
            ...
            <input
              ...
              className="w-full p-3 bg-white focus:ring-2 focus:ring-blue-500 border border-slate-300 rounded-xl outline-none transition-all shadow-sm"
              required
            />
          </div>
          <div className="md:col-span-3">
            ...
            <input
              ...
              placeholder="Acesso..."
              className="w-full p-3 bg-white focus:ring-2 focus:ring-blue-500 border border-slate-300 rounded-xl outline-none transition-all shadow-sm font-mono"
              required
            />
          </div>
          <div className="md:col-span-3">
          ...
          <div className="md:col-span-2">
            <button
              type="submit"
              className={`w-full text-white p-3 rounded-xl font-bold transition-all shadow-sm h-[50px] ${editingId ? "bg-orange-500 hover:bg-orange-600" : "bg-slate-800 hover:bg-slate-900"}`}
            >
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 mb-6 flex flex-col sm:flex-row gap-4 animate-in fade-in slide-in-from-bottom-2">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 p-3 bg-slate-50 focus:bg-white border border-slate-300 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
          />
        </div>
        <div className="sm:w-1/3 relative">
          ...
          <select
            ...
            className="w-full pl-10 p-3 bg-slate-50 focus:bg-white border border-slate-300 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 transition-colors appearance-none cursor-pointer"
          >
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredStudents.map((s) => {
                ...
                return (
                  <tr
                    key={s.id}
                    className={`transition-colors ${isEditingThis ? "bg-orange-50/50" : "hover:bg-slate-50"}`}
                  >
                    ...
                    <td className="p-4 text-sm text-slate-600">
                      {classes.find((c) => c.id === s.classId)?.name || (
                        <span className="text-red-500 italic">Sem Turma</span>
                      )}
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          className="text-red-500 p-2 hover:bg-red-100 rounded-lg transition-colors"
              ...
              {filteredStudents.length === 0 && (
                <tr>
                  <td colSpan={4} className="p-8 text-center text-slate-500">
                    {students.length === 0
                      ? "Nenhum aluno registado na plataforma."
...
function AdminQuizzes({ quizzes }: { quizzes: Quiz[] }) {
  ...
  const addQuestionToQueue = () => {
    if (!question || !opt1 || !opt2 || !opt3 || !opt4)
      return alert("Preencha todos os campos da pergunta!");
    const options = [
  ...
  const handleJsonImport = () => {
    ...
    try {
      ...
      if (!Array.isArray(parsed))
        throw new Error("O formato raiz precisa ser um array (lista).");
      ...
      if (!isValid)
        throw new Error(
          "Alguma pergunta não tem o formato correto (precisa de 'question' e 'options').",
        );
  ...
  const saveFullQuiz = async () => {
    if (!title || questionsList.length === 0)
      return alert("Dê um título e adicione pelo menos uma pergunta!");

    if (editingQuizId) {
      await updateDoc(
        doc(db, "artifacts", appId, "public", "data", "quizzes", editingQuizId),
        {
          moduleId,
          title,
  ...
  const toggleActive = async (id: string, current: boolean) =>
    await updateDoc(
      doc(db, "artifacts", appId, "public", "data", "quizzes", id),
      { isActive: !current },
    );
  const handleDelete = async (id: string) => {
    if (window.confirm("Excluir Quiz Inteiro permanentemente?"))
      await deleteDoc(
        doc(db, "artifacts", appId, "public", "data", "quizzes", id),
      );
  };
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">
          Crie avaliações interativas para os módulos.
        </p>
      </div>

      {/* CONSTRUTOR DE QUIZ */}
      <div
        className={`p-6 rounded-2xl shadow-sm border mb-10 transition-colors duration-300 ${editingQuizId ? "bg-orange-50 border-orange-300" : "bg-white border-slate-200"}`}
      >
        <div className="flex justify-between items-center mb-6 border-b border-slate-200/50 pb-4">
          <h4
            className={`font-bold text-lg flex items-center gap-2 ${editingQuizId ? "text-orange-800" : "text-slate-800"}`}
          >
            {editingQuizId ? (
              <>
                <Pencil size={20} className="text-orange-600" /> A Editar Quiz
                Existente
              </>
            ) : (
              <>
                <ListChecks size={20} className="text-blue-600" /> Criar Novo
                Quiz
              </>
            )}
          </h4>
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          <div className="md:col-span-1">
            ...
            <select
              ...
              className="w-full p-3 border border-slate-300 rounded-xl font-semibold text-blue-900 bg-white focus:ring-2 focus:ring-blue-500 outline-none"
            >
              {MODULES.map((m) => (
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="Ex: Avaliação de Hardware Básico"
              className="w-full p-3 border border-slate-300 rounded-xl font-semibold bg-white focus:ring-2 focus:ring-blue-500 outline-none"
              required
            />
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 mb-6 shadow-sm">
          <div className="flex border-b border-slate-100 mb-5 gap-4">
            <button
              ...
              className={`pb-3 text-sm font-bold border-b-2 transition-colors ${inputMode === "manual" ? "border-blue-600 text-blue-600" : "border-transparent text-slate-400 hover:text-slate-600"}`}
            >
              Adição Manual
            </button>
            <button
              onClick={() => setInputMode("json")}
              className={`pb-3 text-sm font-bold border-b-2 transition-colors flex items-center gap-1 ${inputMode === "json" ? "border-blue-600 text-blue-600" : "border-transparent text-slate-400 hover:text-slate-600"}`}
            >
          ...
          {inputMode === "manual" ? (
            <div>
              <input
                ...
                className="w-full p-4 border border-slate-300 rounded-xl mb-4 bg-slate-50 focus:bg-white outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
              />
              <div className="grid md:grid-cols-2 gap-3 mb-4">
                {[opt1, opt2, opt3, opt4].map((_, i) => (
                  <div
                    key={i}
                    className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${correctIdx === i ? "bg-green-50 border-green-400" : "bg-slate-50 border-slate-200 hover:border-blue-300"}`}
                  >
                    <input
                      type="radio"
                    ...
                    <input
                      type="text"
                      value={
                        i === 0 ? opt1 : i === 1 ? opt2 : i === 2 ? opt3 : opt4
                      }
                      onChange={(e) => {
                        const val = e.target.value;
                        if (i === 0) setOpt1(val);
                        else if (i === 1) setOpt2(val);
                        else if (i === 2) setOpt3(val);
                        else setOpt4(val);
                      }}
                      placeholder={`Opção ${i + 1} ${correctIdx === i ? "(Correta)" : ""}`}
              ...
              <input
                ...
                placeholder="Explicação que o aluno verá ao finalizar..."
                className="w-full p-3 border border-slate-300 rounded-xl mb-4 bg-slate-50 focus:bg-white outline-none focus:ring-2 focus:ring-blue-500 transition-colors text-sm"
              />
              <button
                type="button"
                onClick={addQuestionToQueue}
                className="w-full bg-slate-800 hover:bg-slate-900 text-white px-4 py-3 rounded-xl text-sm font-bold transition-transform active:scale-95 shadow-sm"
              >
                <Plus size={16} className="inline mr-2" /> Adicionar à Lista de
                Perguntas
              </button>
            </div>
          ) : (
            <div>
              <p className="text-xs text-slate-500 mb-2 font-medium">
                Cole um array JSON com as perguntas. Exemplo de formato:
              </p>
    ...
    "options": [
      ...
      { "text": "Parte física", "isCorrect": true }
              ...
              <textarea
                ...
                rows={6}
                className="w-full p-4 border border-slate-300 rounded-xl font-mono text-sm bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none"
                placeholder="Colar JSON aqui..."
              ></textarea>
              <button
                type="button"
                onClick={handleJsonImport}
                className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 rounded-xl text-sm font-bold shadow-sm transition-transform active:scale-95"
              >
                Importar Perguntas
              </button>
            </div>
          ...
          <div className="mb-6 bg-blue-50/50 p-4 rounded-2xl border border-blue-100">
            ...
            <ul className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
              {questionsList.map((q, idx) => (
                <li
                  key={idx}
                  className="text-sm bg-white border border-slate-200 text-slate-700 p-3 rounded-xl flex justify-between items-center shadow-sm"
                >
        ...
        <button
          ...
          disabled={questionsList.length === 0 || !title}
          className={`w-full text-white p-4 rounded-xl font-bold text-lg shadow-md transition-all active:scale-95 disabled:bg-slate-300 disabled:cursor-not-allowed ${editingQuizId ? "bg-orange-500 hover:bg-orange-600" : "bg-green-600 hover:bg-green-700"}`}
        >
          {editingQuizId
            ? "Gravar Alterações do Quiz"
            : "Finalizar e Guardar Novo Quiz"}
        </button>
      </div>

      {/* LISTAGEM DE QUIZZES GUARDADOS */}
      <h4 className="font-black text-slate-800 text-xl mb-4">
        Quizzes Gravados no Sistema
      </h4>
            ...
            >
              ...
              <div className="divide-y divide-slate-100 p-2">
                {modQuizzes.map((q) => (
                  <div
                    key={q.id}
                    className={`p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-xl transition-colors ${!q.isActive ? "bg-slate-50/80 opacity-60" : "hover:bg-slate-50"}`}
                  >
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1 flex items-center gap-1">
                        <BookOpen size={12} /> {q.questions?.length || 0}{" "}
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className={`text-xs px-4 py-2 rounded-full font-bold transition-colors mr-2 ${q.isActive ? "bg-green-100 text-green-700 hover:bg-green-200" : "bg-slate-200 text-slate-600 hover:bg-slate-300"}`}
                      >
                      ...
                      <button
                        ...
                        title="Editar Quiz"
                        className="text-blue-600 p-2 hover:bg-blue-100 rounded-lg bg-blue-50 transition-colors"
                      >
                        <Pencil size={18} />
                      </button>
                      <button
                        ...
                        title="Excluir Quiz"
                        className="text-red-500 p-2 hover:bg-red-100 rounded-lg bg-red-50 transition-colors"
                      >
                        <Trash2 size={18} />
                      </button>
                    </div>
